...
namespace ExampleFramework.TestAdapter.PlatformServices;
...
public class AssemblyResolver : MarshalByRefObject, IDisposable
{
    ...
    {
        DebugEx.Assert(!string.IsNullOrEmpty(path), "'path' cannot be null or empty.");
    ...
    protected virtual Assembly? SearchAssembly(List<string> searchDirectorypaths, string name, bool isReflectionOnly)
    {
        ...
        DebugEx.Assert(requestedName != null && !string.IsNullOrEmpty(requestedName.Name), "AssemblyResolver.OnResolve: requested is null or name is empty!");
